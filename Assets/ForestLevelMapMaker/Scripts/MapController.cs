...
namespace Mkey
{
    ...
    //    public static int currentLevel = 1; // set from this script by clicking on button. Use this variable to load appropriate level.
        ...
        void Start()
        {
            GamePlay.CurrentLevel = PlayerPrefs.GetInt("CurrentLevel", 0);
            topPassedLevel = PlayerPrefs.GetInt("CurrentLevel", 0);
            ...
            for (int i = 0; i < MapLevelButtons.Count; i++)
            {
               ...
               if(SceneLoader.Instance) SceneLoader.Instance.LoadScene(scene, () => { });
                ...
                SetButtonActive(scene, (GamePlay.CurrentLevel == scene || scene == topPassedLevel + 1), (topPassedLevel >= scene));
